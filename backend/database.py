...
from backend.models import (
    Attendee, Character, CharacterArchetype, EventConfig,
    EventSession, Interaction
)
...
DEMO_ATTENDEES: list[Attendee] = [
    Attendee(
        id="att-001", badge_id="NFC-1001", name="Priya Sharma",
        email="priya@techcorp.io", company="TechCorp", role="ML Engineer",
        ...
        xp_points=120, interaction_count=3
    ),
    Attendee(
        id="att-002", badge_id="NFC-1002", name="James Chen",
        email="james@startupx.co", company="StartupX", role="Full-Stack Developer",
        interests=["web development", "React", "Kubernetes", "cloud architecture"],
        sessions_attended=["Cloud-Native Apps", "React 19 Deep Dive"],
        xp_points=80, interaction_count=1
    ),
    Attendee(
        id="att-003", badge_id="NFC-1003", name="Aisha Mohammed",
        email="aisha@datalytics.ai", company="Datalytics", role="Data Scientist",
        ...
        xp_points=200, interaction_count=5
    ),
    Attendee(
        id="att-004", badge_id="NFC-1004", name="Marcus Davis",
        email="marcus@cybershield.com", company="CyberShield", role="Security Analyst",
        ...
        xp_points=50, interaction_count=0
    ),
    Attendee(
        id="att-005", badge_id="NFC-1005", name="Elena Rodriguez",
        email="elena@designlab.io", company="DesignLab", role="UX Designer",
        ...
        xp_points=90, interaction_count=2
    ),
    Attendee(
        id="att-006", badge_id="NFC-1006", name="Raj Patel",
        email="raj@quantum.dev", company="QuantumDev", role="Quantum Computing Researcher",
        ...
        xp_points=150, interaction_count=4
    ),
]
...
DEMO_EVENT = EventConfig(
    ...
    sessions=[
        EventSession(id="s-01", title="Generative AI Keynote", speaker="Dr. Anya Patel",
                     track="AI & ML", time_slot="09:00-10:00", room="Main Hall",
                     description="Exploring the frontier of generative AI and its impact.",
                     tags=["AI", "generative", "keynote"]),
        EventSession(id="s-02", title="Intro to Transformers", speaker="Prof. Liu Wei",
                     track="AI & ML", time_slot="10:30-11:30", room="Room A",
                     description="Deep dive into transformer architecture.",
                     tags=["AI", "transformers", "deep learning"]),
        EventSession(id="s-03", title="Python Best Practices", speaker="Sarah Johnson",
                     track="Development", time_slot="10:30-11:30", room="Room B",
                     description="Modern Python patterns for production code.",
                     tags=["python", "best practices", "coding"]),
        EventSession(id="s-04", title="Cloud-Native Apps", speaker="Mike Torres",
                     track="Cloud", time_slot="11:45-12:45", room="Room A",
                     description="Building scalable applications with Kubernetes.",
                     tags=["cloud", "kubernetes", "architecture"]),
        EventSession(id="s-05", title="React 19 Deep Dive", speaker="Emma Zhang",
                     track="Development", time_slot="11:45-12:45", room="Room B",
                     description="What's new in React 19 and how to migrate.",
                     tags=["react", "frontend", "web"]),
        EventSession(id="s-06", title="Zero Trust Architecture", speaker="Alex Petrov",
                     track="Security", time_slot="14:00-15:00", room="Room A",
                     description="Implementing zero trust in modern enterprises.",
                     tags=["security", "zero trust", "enterprise"]),
        EventSession(id="s-07", title="NLP in Production", speaker="Dr. Maria Santos",
                     track="AI & ML", time_slot="14:00-15:00", room="Room B",
                     description="Deploying NLP models at scale.",
                     tags=["NLP", "ML", "production"]),
        EventSession(id="s-08", title="MLOps Workshop", speaker="David Kim",
                     track="AI & ML", time_slot="15:30-17:00", room="Workshop Lab",
                     description="Hands-on MLOps pipeline building.",
                     tags=["MLOps", "CI/CD", "ML"]),
        EventSession(id="s-09", title="Inclusive Design Workshop", speaker="Lisa Park",
                     track="Design", time_slot="15:30-17:00", room="Design Studio",
                     description="Designing for accessibility and inclusion.",
                     tags=["design", "accessibility", "UX"]),
        EventSession(id="s-10", title="Data Ethics Panel", speaker="Panel Discussion",
                     track="AI & ML", time_slot="14:00-15:00", room="Main Hall",
                     description="Debating the ethical boundaries of AI.",
                     tags=["ethics", "AI", "panel"]),
        EventSession(id="s-11", title="Quantum Computing 101", speaker="Dr. Raj Patel",
                     track="Emerging Tech", time_slot="10:30-11:30", room="Room C",
                     description="Introduction to quantum computing principles.",
                     tags=["quantum", "computing", "physics"]),
        EventSession(id="s-12", title="Design Systems at Scale", speaker="Chris Anderson",
                     track="Design", time_slot="11:45-12:45", room="Design Studio",
                     description="Building and maintaining enterprise design systems.",
                     tags=["design", "systems", "UX"]),
        EventSession(id="s-13", title="Ethical Hacking Workshop", speaker="Natasha Romanov",
                     track="Security", time_slot="15:30-17:00", room="Security Lab",
                     description="Hands-on penetration testing techniques.",
                     tags=["security", "hacking", "ethical"]),
        EventSession(id="s-14", title="Post-Quantum Cryptography", speaker="Dr. Alan Turing Jr.",
                     track="Security", time_slot="11:45-12:45", room="Room C",
...
class InMemoryDatabase:
    ...
    def get_attendee_by_badge(self, badge_id: str) -> Optional[Attendee]:
    ...
    def update_attendee(self, attendee_id: str, data: dict) -> Optional[Attendee]:
    ...
    def list_interactions(self, limit: int = 50) -> list[Interaction]:
        return sorted(
            self.interactions, key=lambda x: x.timestamp, reverse=True
        )[:limit]
...
class FirestoreDatabase:
    ...
    def get_attendee_by_badge(self, badge_id: str) -> Optional[Attendee]:
        docs = (self.db.collection("attendees")
                .where("badge_id", "==", badge_id).limit(1).stream())
        for doc in docs:
            return Attendee(**doc.to_dict())
        return None

    def list_attendees(self) -> list[Attendee]:
        return [Attendee(**d.to_dict())
                for d in self.db.collection("attendees").stream()]

    def create_attendee(self, attendee: Attendee) -> Attendee:
        self.db.collection("attendees").document(attendee.id).set(
            attendee.model_dump()
        )
        return attendee
    ...
    def list_characters(self) -> list[Character]:
        return [Character(**d.to_dict())
                for d in self.db.collection("characters").stream()]

    def create_character(self, character: Character) -> Character:
        self.db.collection("characters").document(character.id).set(
    ...
    # --- Interactions ---
    def add_interaction(self, interaction: Interaction) -> Interaction:
        self.db.collection("interactions").document(interaction.id).set(
